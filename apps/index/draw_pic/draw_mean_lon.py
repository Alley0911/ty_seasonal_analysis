...
def draw_mean_lon(start_year, start_month, end_year, end_month, title):
    ...
    for year in years:
        ...
        for i in (data_tmp):
            ...
            for record in records:
                lon_tmp.append(record.loc['coordinates'][0])
            lon_mean.append(np.nanmean(lon_tmp))
        lon.append(np.nanmean(lon_mean))

    #  求指定年份的平均
    lon_mean_of_selected = [np.nanmean(lon)]*len(lon)
    # print(lon_mean_of_selected)
    # 求气候场平均
    lon_cli = []
