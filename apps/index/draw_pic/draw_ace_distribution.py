...
def draw_ace_distribution(start_year, start_month, end_year, end_month,title):

    # 台风路径密度格点
    lon_grid = np.arange(100, 180.1, 2.5)
    lat_grid = np.arange(0, 90.1, 2.5)
    ace = np.zeros((len(lat_grid), len(lon_grid)), dtype=float)
        ...
        for lon_tmp, lat_tmp in zip(lon, lat):
            for lon_g in lon_grid:
                for lat_g in lat_grid:
                    if abs(lon_tmp - lon_g)<=1.25 and abs(lat_tmp - lat_g)<=1.25:
    ...
    # print(ace.shape)
        for index_lat, lat_ in enumerate(lat_grid):
            for index_lon, lon_ in enumerate(lon_grid):
                for index, loc_ty in enumerate(zip(lon_adjust, lat_adjust)):
                    if loc_ty[0] == lon_ and loc_ty[1] == lat_:
                        ace[index_lat, index_lon] += v[index]**2 * 10**-4 
    # print(ace.max())
    # print(ace.min())
    #########################################################################
    # 1. 画图
    ...
    norm = plt.Normalize(vmin=np.min(ace),vmax=np.max(ace))
    cdict = {
      'red'  :  ( (0.0, 0.25, 1), (0.5,0.5, 0.5), (1., 1., 1.)),
      'green':  ( (0.0, 0.0, 1), (0.5, 0, 0), (1., 0, 0)),
      'blue' :  ( (0.0, 1.0, 1), (0.5, 0, 0), (1., 0, 0))
    }
    ...
    # cn = ax.contourf(lon_grid, lat_grid, ace,cmap='Greens',levels=range(1, max_value+1, 1), transform=ccrs.PlateCarree(),extend='both')
    # rotated_pole = ccrs.RotatedPole(pole_longitude=177.5, pole_latitude=37.5)
    cn = ax.pcolor(lon_grid, lat_grid, ace, shading='nearest', cmap=cm, norm=norm)
    # plt.pcolormesh(ace, shading='flat')
...
if __name__ == "__main__":
    draw_ace_distribution(2019, 1, 2019, 12)
